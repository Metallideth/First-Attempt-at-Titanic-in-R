...
sum(ifelse(predict(logisticreg,select(training,-Survived),type="response")>0.5,1,0) == training$Survived)/dim(training)[1]

sum(ifelse(predict(logisticreg,select(cv,-Survived),type="response")>0.5,1,0) == cv$Survived)/dim(cv)[1]
